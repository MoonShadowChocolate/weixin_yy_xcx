    ...
    if(options.types=='order'){
      ...
      app.request(app.host + 'order/find/' + options.id, 'POST', 'form', {
        ...
        for(let i of data.detailList){
          ...
          item.num=i.quantity;
          var redu=i.discountAmount||0;
          var price=(i.totalPrice-redu).toString();
          if(price.indexOf('.')==-1){
            price+='.00';
          }else if(price.slice(price.indexOf('.')).length<3){
            price+='0';
          }
          item.price = price;
          item.spec=[];
